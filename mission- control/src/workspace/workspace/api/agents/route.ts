...
export async function GET() {
  try {
    ...
    const agents: Array<{ name: string; type: string; lastActive: string; isActive: boolean; context?: { used: number; total: number } | null }> = [];
    ...
    for (const line of lines) {
      ...
      const context = contextMatch ? { used: parseInt(contextMatch[1]), total: parseInt(contextMatch[2]) } : null;
      
      agents.push({ name, type, lastActive, isActive, context });
    }
    ...
    return NextResponse.json({
      ...
      agents,
      summary: { total: agents.length, active: agents.filter(a => a.isActive).length, idle: agents.filter(a => !a.isActive).length }
    });
